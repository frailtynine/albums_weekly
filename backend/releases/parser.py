...
def parse_cos_releases() -> ReleaseInfo:
    """Parses next Friday releases from CoS website."""
...
def parse_sg_releases() -> ReleaseInfo:
    """
    Parses next Friday releases from Stereugum.

    Releases are parsed from Album of the Week page.
    """
    release_info: ReleaseInfo = ReleaseInfo()
    ...
    title_text = article_soup.find('h1', class_='review-hero__title headline')
    album_of_the_week = f'Steregum {title_text.text}'
...
def parse_releases() -> ReleaseInfo:
    cos_releases = parse_cos_releases()
    sg_releases = parse_sg_releases()
    combined_list = combine_lists(cos_releases, sg_releases)
